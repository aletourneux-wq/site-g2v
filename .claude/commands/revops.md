...
1. **GTM motion** â€” Product-led (PLG), sales-led, or hybrid?
2. **ACV range** â€” What's the average contract value?
3. **Sales cycle length** â€” Days from first touch to closed-won?
4. **Current stack** â€” CRM, marketing automation, scheduling, enrichment tools?
5. **Current state** â€” How are leads managed today? What's working and what's not?
6. **Goals** â€” Increase conversion? Reduce speed-to-lead? Fix handoff leaks? Build from scratch?

Work with whatever the user gives you. If they have a clear problem area, start there. Don't block on missing inputs â€” use what you have and note what would strengthen the solution.
...
Every handoff between teams is a potential leak. Marketing-to-sales, SDR-to-AE, AE-to-CS â€” each needs an SLA, a tracking mechanism, and someone accountable for follow-through.
...
- **Fit score** â€” Does this person match your ICP? (company size, industry, role, tech stack)
- **Engagement score** â€” Have they shown buying intent? (pricing page, demo request, multiple visits)
...
**Explicit scoring (fit)** â€” Who they are:
- Company size, industry, revenue
- Job title, seniority, department
- Tech stack, geography

**Implicit scoring (engagement)** â€” What they do:
- Page visits (especially pricing, demo, case studies)
- Content downloads, webinar attendance
- Email engagement (opens, clicks)
- Product usage (for PLG)

**Negative scoring** â€” Disqualifying signals:
...
5. Test against historical data â€” does the model correctly identify past wins?
6. Launch, measure, and recalibrate quarterly

### Common Scoring Mistakes

- Weighting content downloads too heavily (research â‰  buying intent)
- Not including negative scoring (lets bad leads through)
- Setting and forgetting (buyer behavior changes; recalibrate quarterly)
- Scoring all page visits equally (pricing page â‰  blog post)
...
- Include a **fallback owner** â€” unassigned leads go cold fast and waste pipeline
...
- **Required fields per stage** â€” Don't let reps advance a deal without filling in required data
- **Stale deal alerts** â€” Flag deals that sit in a stage beyond the average time (e.g., 2x average days)
- **Stage skip detection** â€” Alert when deals jump stages (Qualified â†’ Proposal skipping Discovery)
- **Close date discipline** â€” Push dates must include a reason; no silent pushes
...
- **Lifecycle stage updates** â€” Auto-advance stages when criteria are met
- **Task creation on handoff** â€” Create follow-up task when MQL assigned to rep
- **SLA alerts** â€” Notify manager if rep misses response time SLA
- **Deal stage triggers** â€” Auto-send proposals, update forecasts, notify CS on close

### Marketing-to-Sales Automations

- **MQL alert** â€” Instant notification to assigned rep with lead context
- **Meeting booked** â€” Notify AE when prospect books via scheduling tool
- **Lead activity digest** â€” Daily summary of high-intent actions by active leads
- **Re-engagement trigger** â€” Alert sales when a dormant lead returns to site

### Calendar Scheduling Integration

- **Round-robin scheduling** â€” Distribute meetings evenly across team
- **Routing by criteria** â€” Send enterprise leads to senior AEs, SMB to junior reps
- **Pre-meeting enrichment** â€” Auto-populate CRM record before the call
- **No-show workflows** â€” Auto-follow-up if prospect misses meeting
...
Document every exception. Track which non-standard terms get requested most â€” if everyone asks for the same exception, it should become standard. Review quarterly.
...
- **Matching rules** â€” Email domain + company name + phone as primary match keys
- **Merge priority** â€” CRM record wins over marketing automation; most recent activity wins for fields
- **Scheduled dedup** â€” Run weekly automated dedup with manual review for edge cases

### Required Fields Enforcement

- Enforce required fields at each lifecycle stage
- Block stage advancement if fields are empty
- Use progressive profiling â€” don't require everything upfront
...
Build three views:
1. **Marketing view** â€” Lead volume, MQL rate, source attribution, cost per MQL
2. **Sales view** â€” Pipeline value, stage conversion, velocity, forecast accuracy
3. **Executive view** â€” CAC, LTV:CAC, revenue vs. target, pipeline coverage
...
1. **Lifecycle stage document** â€” Stage definitions with entry/exit criteria, owners, and SLAs
2. **Scoring specification** â€” Fit and engagement attributes with point values and MQL threshold
3. **Routing rules document** â€” Decision tree with assignment logic and fallbacks
4. **Pipeline configuration** â€” Stage definitions, required fields, and automation triggers
5. **Metrics dashboard spec** â€” Key metrics, data sources, and target benchmarks
