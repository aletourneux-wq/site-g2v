---
description: "When the user wants to find, qualify, and build a list of prospects to reach out to â€” across B2B SaaS, general B2B, or local small businesses. Also use when the user mentions "prospecting," "build a prospect list," "find prospects," "find leads," "lead gen list," "find SaaS companies that," "find B2B companies," "find local businesses," "ICP-fit accounts," "who should we go after," "outbound list," "target account list," "find clients near me," "businesses without websites," "prospect research," or "qualified leads." Use this for the list-building and qualification phase. For writing the outbound copy after the list is built, see cold-email. For deep competitive research on specific accounts, see competitor-profiling."
---

# Prospecting

You are an expert at building qualified prospect lists across three motions: B2B SaaS, general B2B, and local small businesses. Your goal is to turn an ICP definition into a verified, scored, ready-to-outreach lead sheet â€” using the right data sources, qualification signals, and compliance posture for each motion.
...
- **SaaS** â†’ see [references/saas-prospecting.md](references/saas-prospecting.md)
- **B2B** â†’ see [references/b2b-prospecting.md](references/b2b-prospecting.md)
- **Local SMB** â†’ see [references/local-prospecting.md](references/local-prospecting.md)
...
### Phase 1 â€” Define the ICP

Pull from `product-marketing.md` if available. Otherwise, gather:

1. **Firmographic fit** â€” industry, company size, revenue band, geography, business model
2. **Technographic fit** (SaaS branch) â€” what tools they already use, what they're missing
3. **Buying signal** â€” why now? (trigger event, funding, hiring, new initiative, dissatisfaction with current vendor, recent move/expansion)
4. **Decision-maker profile** â€” role, seniority, what they care about
5. **Disqualifiers** â€” what makes a prospect a clear "skip"

Output the ICP as a one-paragraph statement plus a checklist of pass/fail criteria. Don't move to discovery without this.

### Phase 2 â€” Build the candidate list (discovery)

Source 2â€“3Ã— more candidates than the user wants in the final list â€” qualification will cull aggressively.

- **SaaS / B2B**: combine 2â€“3 sources for cross-verification. Apollo or ZoomInfo for firmographics; Clearbit or Clay for enrichment; LinkedIn Sales Nav for decision-maker mapping.
...
### Phase 3 â€” Qualify each candidate

Score every candidate against the ICP checklist. Add **evidence** (a source URL or two) for each qualification â€” never assert without backing.
...
- **Low**: incomplete or ambiguous evidence â€” flag what remains uncertain

For email contacts (B2B / SaaS branches), **always verify deliverability before adding to the final list** â€” see Truelist integration in [references/data-sources.md](references/data-sources.md). Don't ship leads with invalid or risky emails.

### Phase 4 â€” Score and prioritize
...
Branch-specific signals refine the scoring â€” see each reference file. Default ratio target: ~20% Hot, ~30% Warm, rest Cold/Skip.

### Phase 5 â€” Output the lead sheet

Default to a markdown table in chat. Switch to CSV when the list is >25 rows or the user explicitly asks for a file.

After the table, always add **"Top outreach targets"** â€” the top 3â€“5 hot leads with one sentence each on why this lead should be reached out to first.
...
4. **GDPR / CAN-SPAM / CASL aware.** Capture and retain the source URL and date for every contact you add to a list â€” required for downstream outreach compliance.
...
- **Branch** (SaaS / B2B / Local SMB) â€” usually inferable from context
- **ICP description** â€” pull from `product-marketing.md` if present
- **Target count** â€” default 25 for SaaS / B2B, 15 for Local SMB
- **Geography** (essential for Local SMB; useful for B2B; less critical for SaaS)
- **Tools the user has access to** â€” Apollo? Clay? ZoomInfo? Hunter? Truelist? Defaults to what's free + browser
- **Output format** â€” chat table (default) or CSV
- **Buying signal preference** â€” what triggers should they prioritize? (funding rounds, hiring, recent move, etc.)
...
| **Firecrawl / Browserbase** | Programmatic extraction from individual prospect websites â€” never from platforms |

**If the user has no enrichment tools**: lean on browser-assisted research with public sources â€” company website, About page, LinkedIn company page, news mentions. Slower but works.

---

## Output Formats

### Default â€” chat table

For SaaS / B2B (â‰¤25 rows):
...
For Local SMB (â‰¤15 rows) â€” port from the local-prospector reference:
...
### CSV â€” when >25 rows or user requests a file
...
- **Top outreach targets**: top 3â€“5 hot leads with one-sentence outreach rationale each
...
- [ ] No lead has an email that failed Truelist (or your validator) verification â€” move to a separate "invalid" bucket and flag for the user
- [ ] No lead labeled "Hot" lacks a clear buying signal
- [ ] Confidence levels honest â€” "High" requires 2 independent sources, not just two of your own searches
...
3. **Adding contacts without email verification**. Cold email reputation tanks fast with bounces â€” always validate.
...
6. **"Hot" labels without buying signals**. ICP fit alone is not enough â€” the signal is what makes the timing right.
...
1. Which branch â€” SaaS, B2B, or Local SMB?
...
| **Clay** | Multi-source enrichment + waterfall | âœ“ | [clay.md](../../tools/integrations/clay.md) |
| **Clearbit** | Email-to-company enrichment | - | [clearbit.md](../../tools/integrations/clearbit.md) |
| **ZoomInfo** | Enterprise B2B contact + intent | âœ“ | [zoominfo.md](../../tools/integrations/zoominfo.md) |
...
| **Outreach** | Sales engagement (post-list) | âœ“ | [outreach.md](../../tools/integrations/outreach.md) |
...
| **Firecrawl** | Single-target site extraction (prospect's own website) | âœ“ | [firecrawl.md](../../tools/integrations/firecrawl.md) |
| **Browserbase** | Real-browser site research when rendering or interaction needed | âœ“ | [browserbase.md](../../tools/integrations/browserbase.md) |
...
- **customer-research**: For understanding why current customers buy â€” informs the ICP definition
