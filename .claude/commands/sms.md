---
description: "When the user wants to plan, build, or optimize SMS or MMS marketing â€” including welcome flows, abandoned cart texts, post-purchase, win-back, promotional sends, or transactional/auth SMS. Also use when the user mentions "SMS marketing," "text message campaigns," "SMS sequence," "SMS automation," "abandoned cart text," "post-purchase SMS," "Klaviyo SMS," "Postscript," "Attentive," "Twilio," "A2P 10DLC," "TCPA," "SMS compliance," "short code," "toll-free SMS," "MMS campaign," "should I do SMS," or "SMS vs email." For email sequences, see emails. For SMS copy framing, see copywriting. For opt-in popups that capture phone numbers, see popups."
---
...
- Geographic mix (US, EU, both â€” compliance differs dramatically)
...
- US: A2P 10DLC registration complete? (Required since 2022 â€” without it, your messages get filtered)
...
## Compliance â€” Read First

**Compliance is the foundation, not an afterthought.** A single TCPA class-action settlement runs $5Mâ€“$40M. The basics:

### US â€” TCPA (Telephone Consumer Protection Act)
...
5. **Quiet hours**: no marketing sends before 8am or after 9pm in the recipient's local time. Carrier rules and state laws (e.g., Florida, Oklahoma, Washington) are stricter than federal â€” default to 9amâ€“8pm recipient-local.
6. **Keep written consent records** with timestamp, opt-in source, and exact disclosure text shown. Auditable.

### US â€” A2P 10DLC Registration (required since 2022)
...
|------|-----------|------|----------|-------|
| **Short code (5-6 digit)** | 100+ msg/sec | $500â€“$1,000/mo + setup | High-volume marketing | Highest (carrier-vetted) |
| **Toll-free (1-8XX)** | ~3 msg/sec | $10â€“$30/mo | Mid-volume, B2C support | Medium-high (carrier-verified) |
| **10DLC (regular long code)** | 1â€“250 msg/sec | $2â€“$10/mo | SMB, conversational, transactional | Medium (requires A2P 10DLC reg) |

**Rule of thumb**: list <10K = 10DLC. List 10Kâ€“100K = toll-free. List 100K+ = short code.
...
SMS isn't free. At $0.0075â€“$0.04 per send + carrier fees, a 100K send costs $750â€“$4,000. This forces relevance â€” you can't "blast." Segment hard.

### 2. Opt-in is your most valuable asset
Opt-in rate from email â†’ SMS is typically 5â€“25%. A high-quality SMS list of 10K beats a low-quality list of 100K. Optimize opt-in quality, not volume.
...
"From [Brand]:" or branded short code at the start of every message. Even on automated flows. Recipients can't see "from" address â€” they need it inline.
...
- Send 1 (60â€“90 days after last purchase): "We miss you" + curated picks
...
### Structure
1. **Sender ID** ("From Acme:" or brand short code) â€” required
2. **Hook** â€” first 5 words decide if they read on
3. **Value** â€” what's in it for them, specifically
4. **CTA + short link** â€” single action, single URL
5. **Compliance footer** â€” "Reply STOP to opt out" (required on opt-in confirmation and at least quarterly thereafter; carrier-recommended on every promotional message)

### Length

- **160 chars (GSM-7)** = 1 segment. Aim here.
- **70 chars (UCS-2)** if you use emojis, accented characters, or curly quotes â€” you'll pay for more segments.
- **161â€“306 chars** = 2 segments (concatenated SMS). Acceptable for richer messages, but you're paying double per send.
- **MMS** (image + up to 1,600 chars) = 3â€“5Ã— the SMS cost. Use sparingly for high-impact moments.

### Voice

- Conversational, not corporate. SMS feels personal â€” write like you're texting a friend.
...
- Don't fake intimacy ("Hey friend!") â€” it backfires
...
|----------|----------|:---:|-----------|
| **Klaviyo SMS** | DTC ecom already on Klaviyo email | âœ“ | $$ |
...
| **Brevo SMS** | EU-focused, email + SMS combo | âœ“ | $ |
| **SimpleTexting** | SMB, simple needs, ease of use | - | $ |
| **Customer.io** | Behavior-based automation + SMS | - | $$ |

**Quick picks**:
- Already on Klaviyo for email + DTC/ecom â†’ **Klaviyo SMS** (no second platform to learn)
- Shopify ecom, want deeper SMS-specific features â†’ **Postscript**
- Building custom SMS into a product â†’ **Twilio**
- B2B SaaS doing transactional/auth â†’ **Twilio** or **Customer.io**
...
|--------|-------------------|--------------------------|
| **Opt-in rate** | Top of funnel health | 5â€“25% of email subscribers |
| **CTR** | Message relevance | 8â€“15% (vs ~3% email) |
| **Conversion rate (per send)** | Revenue impact | 1â€“5% per promotional send |
| **Revenue per send (RPS)** | Channel economics | $0.20â€“$2.00 |
| **Opt-out rate per send** | Audience fatigue | <2% per send, <0.5% for promotional |
| **Cost per send** | Channel cost discipline | $0.0075â€“$0.04 |
| **List growth rate** | Audience momentum | 5â€“15%/month early, 1â€“3% steady-state |
...
- LTV impact: SMS subscribers vs email-only subscribers (typically 1.5â€“3Ã— LTV for SMS opt-ins)
...
Keep recommendations specific. Don't say "send an SMS at the right time" â€” say "send 30 min after cart abandon, 4 hours later if no purchase, 24 hours later with discount."
...
1. **Skipping A2P 10DLC registration** â€” your messages get filtered into oblivion. Register first, send second.
2. **Treating SMS like email** â€” sending daily promotional blasts. Opt-out rates spike, list dies.
3. **Discount on first abandoned cart message** â€” trains customers to always abandon. Reserve for second or third send.
4. **Generic "From: [shortcode]"** â€” recipients need brand name in the message itself.
5. **Forgetting quiet hours** â€” sending at 6 AM local time gets opt-outs and TCPA complaints.
6. **No STOP/HELP handling** â€” non-negotiable. Every platform handles this; verify yours does.
7. **Emojis everywhere** â€” pushes you into UCS-2 encoding, halves segment size, doubles cost.
8. **Mismatching A2P sample messages and actual sends** â€” carriers flag and block.
9. **Not tracking conversions** â€” you can't justify channel ROI without attribution.
10. **No throttling on bulk sends** â€” burst sends trigger carrier filtering. Use platform throttling.
...
|------|----------|:---:|-------|
| **Klaviyo** | E-commerce email + SMS combined | âœ“ | [klaviyo.md](../../tools/integrations/klaviyo.md) |
...
| **Brevo** | EU email + SMS, SMB-friendly | âœ“ | [brevo.md](../../tools/integrations/brevo.md) |
...
- **emails**: Sister channel â€” almost always run together. Email carries the long-form content; SMS carries the urgent nudges.
